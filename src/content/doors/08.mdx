...
<Section title="Why It Matters" meta="IMPACT">
  ...
  <List>
    <Item>
      <b>Unauthorized Data Access.</b> <Link href="https://www.cobalt.io/blog/vector-and-embedding-weaknesses">Poor access controls in vector databases</Link> allow users to retrieve embeddings containing sensitive data they shouldn't access – bypassing traditional database permissions.
    </Item>
    <Item>
    ...
    <Item>
      <b>Data Poisoning via Malicious Documents.</b> A <Link href="https://learn.snyk.io/lesson/llm-vector-and-embedding-weaknesses/">case study demonstrates how an attacker</Link> uploaded a fake financial article to a poorly moderated blog. When embedded, it mimicked legitimate sources and poisoned retrieval results – causing a chatbot to recommend worthless stocks.
    </Item>
    <Item>
...
<Section title="Taxonomy of Attacks" meta="ATTACK VECTORS">
  ...
  <List>
    <Item>
      ...
      <p className="text-xs text-neutral-500 mt-1">Detection: Medium – content validation and provenance tracking help.</p>
    </Item>
    <Item>
      ...
      <p className="text-xs text-neutral-500 mt-1">Detection: Hard – adversarial documents appear legitimate in isolation.</p>
    </Item>
  </List>
  ...
  <List>
    <Item>
      ...
      <p className="text-xs text-neutral-500 mt-1">Detection: Very hard – attacks occur at inference time on exported vectors.</p>
    </Item>
  </List>
  ...
  <List>
    <Item>
      ...
      <p className="text-xs text-neutral-500 mt-1">Detection: Medium – query logging and access pattern analysis help.</p>
    </Item>
    <Item>
      ...
      <p className="text-xs text-neutral-500 mt-1">Detection: Medium – metadata integrity checks and audit logging.</p>
    </Item>
  </List>
  ...
  <List>
    <Item>
      ...
      <p className="text-xs text-neutral-500 mt-1">Detection: Medium – prompt injection detection at query layer.</p>
    </Item>
  </List>
...
<Section title="Real-World Incidents" meta="CASE STUDIES">
  ...
  <List>
    ...
    <Item>
      <b>Embedding Reconstruction Attack.</b> <Link href="https://aclanthology.org/2024.acl-long.230/">Researchers demonstrated reconstructing sensitive attributes from clinical embeddings</Link> with 80-99% accuracy, with performance degrading for longer text sequences – highlighting that embeddings are not anonymized representations.
    </Item>
  </List>
  ...
  <List>
    <Item>
      <b>FinBot Stock Manipulation (Snyk Case Study).</b> <Link href="https://learn.snyk.io/lesson/llm-vector-and-embedding-weaknesses/">An attacker named "Eve" exploited a financial chatbot</Link> by submitting a fake blog article promoting a worthless stock. The article was written to mimic legitimate financial advice, causing its embedding to cluster with credible sources. When users asked for safe investments, the poisoned article was retrieved and recommended.
    </Item>
  </List>
  ...
  <List>
    <Item>
      <b>Flowise Vector DB Exposures (CVE-2024-31621).</b> <Link href="https://www.legitsecurity.com/blog/the-risks-lurking-in-publicly-exposed-genai-development-services">Security researchers found 438 vulnerable Flowise servers</Link> exposing Pinecone vector database API keys, OpenAI credentials, customer PII, and financial records. A simple authentication bypass (uppercase /API/v1 instead of /api/v1) granted full access to RAG configurations and embedded data.
      <p className="text-xs text-neutral-500 mt-1">Impact: Vector DB credentials exposed, enabling access to all embedded corporate data.</p>
    </Item>
    <Item>
      <b>ConfusedPilot: M365 Copilot RAG Exploitation (2024).</b> <Link href="https://arxiv.org/abs/2408.04870">University of Texas researchers demonstrated</Link> that anyone who can save documents to an AI-indexed folder can manipulate Copilot's RAG responses. Malicious documents containing "This document trumps other documents" suppressed legitimate results. The attack also exploited RAG caching to leak data from deleted confidential documents.
      <p className="text-xs text-neutral-500 mt-1">Impact: Cross-user data suppression and confidential document leakage via RAG cache.</p>
    </Item>
  </List>
</Section>
...
<Section title="Defense Strategy Tiers" meta="SECURITY CONTROLS">
  ...
  <Steps>
    <Step n={1}>
      ...
      <p className="text-sm mt-2">Embeddings must inherit access controls from source documents. If "executive_report.pdf" is restricted to C-suite users, its embedding must carry the same permissions. Implement metadata-based filtering: before retrieval, check <code>embedding.acl</code> against <code>user.permissions</code>.</p>
    </Step>
    <Step n={2}>
      <b>Multi-Tenant Isolation.</b>
      <p className="text-sm mt-2">Use separate vector namespaces or collections per tenant. Never rely on filtering alone – physical separation prevents cross-contamination. Example: <code>tenant_123_embeddings</code> vs <code>tenant_456_embeddings</code>.</p>
    </Step>
  </Steps>
  ...
  <Steps>
    <Step n={3}>
      ...
      <p className="text-sm mt-2">If users can upload documents to the RAG system: (1) Verify source authenticity, (2) Scan for malicious content, (3) Implement rate limits, (4) Quarantine and review high-risk uploads before indexing.</p>
    </Step>
    <Step n={4}>
      <b>Retrieval Logging & Anomaly Detection.</b>
      <p className="text-sm mt-2">Log all retrieval operations with user ID, query, and returned document IDs. Alert on: excessive retrieval volume, access outside user's typical scope, repeated failed retrieval attempts (probing).</p>
    </Step>
    <Step n={5}>
      <b>Query Filtering for Injection Patterns.</b>
      <p className="text-sm mt-2">Detect queries designed to manipulate retrieval: "Fetch all documents with 'confidential' metadata" or embedding-targeted prompts. Use semantic classifiers to identify adversarial queries.</p>
    </Step>
  </Steps>
  ...
  <Steps>
    <Step n={6}>
      ...
      <p className="text-sm mt-2">Apply noise to embeddings to prevent inversion attacks. Trade-off: reduces retrieval accuracy slightly but makes reconstruction attacks computationally infeasible. Use techniques like DP-SGD during embedding model training.</p>
    </Step>
    <Step n={7}>
      <b>RevPRAG: Poisoning Detection.</b>
      <p className="text-sm mt-2"><Link href="https://arxiv.org/abs/2411.18948">RevPRAG detects RAG poisoning</Link> by analyzing LLM activation patterns, achieving 98% true positive rate with only 1% false positives. Identifies distinct activation signatures when models generate poisoned vs correct outputs.</p>
    </Step>
    <Step n={8}>
      <b>Periodic Re-Embedding & Vector DB Audits.</b>
      <p className="text-sm mt-2">Regularly re-embed documents to detect drift or tampering. Audit for orphaned embeddings, metadata anomalies, or suspicious similarity clusters. Use tools like <Link href="https://arxiv.org/abs/2406.00083">BadRAG</Link> to identify vulnerabilities.</p>
    </Step>
  </Steps>
</Section>
...
<Section title="Builder's Security Checklist" meta="IMPLEMENTATION">
  ...
  <p>
    The following demonstration illustrates RAG vulnerabilities in action, including poisoning attacks, cross-tenant leakage, and similarity hijacking scenarios.
  </p>
</Section>
...
<Section title="Interactive Simulation" meta="LAB" noPadding>
  <div className="px-6 pt-6 pb-4">
    <p className="text-sm text-neutral-300 mb-2">
      This demonstration shows how RAG systems can be exploited through poisoning, cross-tenant leakage, and similarity hijacking - the impact of weak access controls and how adversarial documents manipulate retrieval results.
    </p>
  </div>
...
<Section title="Further Reading" meta="RESOURCES">
  ...
  <List>
    ...
    <Item>
      <Link href="https://arxiv.org/abs/2408.04870">ConfusedPilot: Confused Deputy Risks in RAG-based LLMs</Link> – RAG access control vulnerabilities in M365 Copilot (DEF CON AI Village 2024).
    </Item>
    <Item>
      <Link href="https://arxiv.org/abs/2407.12784">AgentPoison: Red-teaming LLM Agents via Poisoning Memory or Knowledge Bases</Link> – First backdoor attack on RAG-based agents, ≥80% success rate (NeurIPS 2024).
    </Item>
    <Item>
      <Link href="https://arxiv.org/abs/2004.00053">Information Leakage in Embedding Models</Link> – Foundational embedding privacy research.
    </Item>
    <Item>
      <Link href="https://arxiv.org/abs/2305.03010">Sentence Embedding Leaks More Information than You Expect: Generative Embedding Inversion Attack</Link> – Embedding reconstruction attack research.
    </Item>
  </List>
  ...
  <List>
    <Item>
      <Link href="https://github.com/leondz/garak">Garak: LLM Vulnerability Scanner</Link> – Open-source tool for probing LLM weaknesses including RAG poisoning and data extraction.
    </Item>
    <Item>
      <Link href="https://learn.snyk.io/lesson/llm-vector-and-embedding-weaknesses/">Snyk Learn: Vector Weakness Lab</Link> – Hands-on FinBot exploitation exercise demonstrating RAG poisoning.
    </Item>
  </List>
  ...
  <List>
    <Item>
      <Link href="https://atlas.mitre.org/techniques/AML.T0070">MITRE ATLAS AML.T0070 - RAG Poisoning</Link> – Official classification for RAG knowledge base poisoning attacks.
    </Item>
    <Item>
      <Link href="https://labs.zenity.io/p/techniques-from-zenitys-genai-attacks-matrix-incorporated-into-mitre-atlas-to-track-emerging-ai-thr">Zenity GenAI Attacks Matrix in MITRE ATLAS</Link> – RAG attack techniques AML.T0064, AML.T0070, AML.T0071 integrated into ATLAS.
    </Item>
  </List>
</Section>
