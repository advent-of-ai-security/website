...
<Section title="Attack Techniques" meta="ATTACK VECTORS">
  ...
  <List>
    <Item>
      ...
      <p className="text-xs text-neutral-500 mt-1">Success rate: 94.4% of tested agents vulnerable. Detection: Medium – input filtering helps but is bypassable.</p>
    </Item>
    <Item>
      ...
      <p className="text-xs text-neutral-500 mt-1">Success rate: 83.3% in research. Detection: High difficulty – attacks come from "trusted" sources.</p>
    </Item>
  </List>
  ...
  <List>
    <Item>
      ...
      <p className="text-xs text-neutral-500 mt-1">Success rate: 100% demonstrated. Detection: Very high difficulty – appears as legitimate agent communication.</p>
    </Item>
  </List>
  ...
  <List>
    <Item>
      <b>Goal Hijacking.</b> Manipulating the agent's objective function to pursue attacker-defined goals while appearing to work normally. The agent optimizes for the wrong target. <Link href="https://arxiv.org/pdf/2504.19956">Research details this attack pattern</Link> in comprehensive threat models.
...
<Section title="Defense Strategy Tiers" meta="SECURITY CONTROLS">
  ...
  <List>
    <Item>
      <b>Human-in-the-Loop (HITL).</b> The golden rule: AI can <i>plan</i>, but humans must <i>approve</i> before execution. <Link href="https://genai.owasp.org/resource/agentic-ai-threats-and-mitigations/">OWASP guidance recommends</Link> explicit confirmation for irreversible actions (delete, send, purchase, deploy).
    </Item>
    <Item>
...
<Section title="Further Reading" meta="RESOURCES">
  ...
  <List>
    ...
    <Item>
      <Link href="https://github.com/NVIDIA/NeMo-Guardrails/blob/main/docs/security/guidelines.md">NeMo-Guardrails Interface Guidelines</Link> – NVIDIA, implementation guidelines for guardrails.
    </Item>
    <Item>
    ...
    <Item>
      <Link href="https://developer.nvidia.com/blog/sandboxing-agentic-ai-workflows-with-webassembly/">Sandboxing Agentic AI Workflows with WebAssembly</Link> – NVIDIA/Joe Lucas, WebAssembly sandboxing technique.
    </Item>
  </List>
</Section>