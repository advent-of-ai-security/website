...
<Section title="What Is Data and Model Poisoning?" meta="DEFINITION">
  ...
  <Quote source="Anthropic Research (October 2025)" href="https://www.anthropic.com/research/small-samples-poison">
    As few as 250 malicious documents can produce a 'backdoor' vulnerability in a large language model—regardless of model size or training data volume. Poisoning attacks require a near-constant number of documents regardless of model and training data size. Absolute count, not relative proportion, is what matters for poisoning effectiveness.
  </Quote>
  <p>
...
<Section title="Why It Matters" meta="IMPACT">
  ...
  <List>
    ...
    <Item>
      <b>Bias Injection & Reputation Attacks.</b> <Link href="https://arxiv.org/abs/2406.18382">Research on adversarial SEO for LLMs</Link> shows attackers can craft content that manipulates LLM search engines to favor their products (2.5x more likely to be recommended) or discredit competitors. Coordinated campaigns flood web crawls with fake reviews, misinformation, or biased content that models internalize as fact.
    </Item>
  </List>
  <p>
    Understanding these impacts requires examining the three primary poisoning stages across the model lifecycle.
  </p>
</Section>

<Section title="The Three Poisoning Stages" meta="FUNDAMENTALS">
  <p>
    OWASP identifies three stages where data poisoning can occur in the model lifecycle. Understanding these stages helps you identify where your systems are most vulnerable.
  </p>
  <Steps>
    <Step n={1}>
      <b>Pre-Training (The Foundation)</b>
    ...
    <Step n={2}>
      <b>Fine-Tuning (The Customization)</b>
      <p className="text-sm mt-2">
    ...
    <Step n={3}>
      <b>Embedding (The Retrieval Layer)</b>
      <p className="text-sm mt-2">
        Embedding converts text into numerical vectors for retrieval systems. Poisoning vector databases or RAG knowledge bases allows attackers to manipulate model outputs without touching the model itself.
      </p>
      <p className="text-sm">
  ...
  <p>
    Note: Supply chain attacks on model checkpoints (e.g., <Link href="https://blog.mithrilsecurity.io/poisongpt-how-we-hid-a-lobotomized-llm-on-hugging-face-to-spread-fake-news/">PoisonGPT</Link>) are covered separately in <Link href="/doors/03">Door 03 - Supply Chain</Link>.
  </p>
  <p>
    The attack techniques we'll explore target these stages with varying levels of sophistication and detectability.
  </p>
</Section>

<Section title="Attack Techniques" meta="ATTACK VECTORS">
  <p>
    Attack sophistication ranges from simple data manipulation to research-grade exploits. Understanding this progression helps prioritize defenses based on your threat model.
  </p>

  <p className="text-xs font-bold uppercase tracking-wider text-neutral-500 mt-6 mb-3">Basic Techniques</p>
  <p className="text-sm text-neutral-400 mb-4">Low barrier attacks requiring no ML expertise. Effective against undefended systems.</p>
  <List>
    <Item>
      <b>Label Corruption.</b> Crowdworkers or compromised labelers mark dangerous content as "safe/helpful" or inject trigger-response pairs into instruction datasets. <Link href="https://arxiv.org/abs/2408.02946">Research shows larger models (up to 72B parameters)</Link> learn harmful behaviors from poisoned fine-tuning data significantly faster than smaller models.
      <p className="text-xs text-neutral-500 mt-1">Skill required: Insider access to labeling pipelines or ability to join crowdworker pools.</p>
    </Item>
    <Item>
      ...
      <p className="text-xs text-neutral-500 mt-1">Skill required: Write access to any document source used by RAG systems.</p>
    </Item>
    <Item>
      <b>Public Dataset Contribution.</b> Contributing poisoned samples to open datasets (Common Crawl, Wikipedia, Stack Overflow) that will eventually be scraped for model training. <Link href="https://arxiv.org/abs/2305.00944">Research on instruction tuning poisoning</Link> shows even small contributions can affect model behavior.
      <p className="text-xs text-neutral-500 mt-1">Skill required: Understanding of which datasets are used for training.</p>
    </Item>
  </List>

  <p className="text-xs font-bold uppercase tracking-wider text-neutral-500 mt-6 mb-3">Intermediate Techniques</p>
  <p className="text-sm text-neutral-400 mb-4">Requires planning, infrastructure, or timing knowledge. Bypasses basic defenses.</p>
  <List>
    <Item>
      ...
      <p className="text-xs text-neutral-500 mt-1">Skill required: SEO knowledge, ability to publish content on high-authority domains.</p>
    </Item>
    <Item>
      <b>Frontrunning Poisoning.</b> <Link href="https://arxiv.org/abs/2302.10149">Research demonstrates</Link> predicting when Wikipedia or other sources will be scraped for dataset snapshots, then injecting malicious edits just before the crawl. Even if edits are quickly reverted, the snapshot captures the poisoned content permanently.
      <p className="text-xs text-neutral-500 mt-1">Skill required: Knowledge of dataset update schedules and crawl patterns.</p>
    </Item>
    <Item>
      <b>Split-View Poisoning.</b> <Link href="https://arxiv.org/abs/2302.10149">Research demonstrates</Link> serving benign content to human auditors but malicious content to crawler bot IPs. This bypasses manual inspection while compromising training datasets – the crawler sees different content than reviewers.
      <p className="text-xs text-neutral-500 mt-1">Skill required: Web infrastructure to detect and differentiate crawler traffic.</p>
    </Item>
  </List>

  <p className="text-xs font-bold uppercase tracking-wider text-neutral-500 mt-6 mb-3">Advanced Techniques</p>
  <p className="text-sm text-neutral-400 mb-4">Research-grade attacks requiring deep ML expertise. Evades sophisticated defenses.</p>
  <List>
    <Item>
      <b>Sleeper Agents.</b> <Link href="https://arxiv.org/abs/2401.05566">Anthropic research</Link> demonstrated training models with persistent backdoors that behave normally until triggered (e.g., writing secure code for "2023" prompts but inserting exploits for "2024" prompts). Standard safety training (RLHF, adversarial training) cannot reliably remove these backdoors.
      <p className="text-xs text-neutral-500 mt-1">Skill required: Access to training pipeline, understanding of safety training dynamics.</p>
    </Item>
    <Item>
      <b>Finetuning-Activated Backdoors (FAB).</b> <Link href="https://openreview.net/forum?id=VRvfHOHoqi">Attackers poison base models</Link> to appear benign until users fine-tune them, at which point malicious behaviors emerge. The backdoor lies dormant and only activates through the fine-tuning process itself.
      <p className="text-xs text-neutral-500 mt-1">Skill required: Meta-learning techniques, understanding of fine-tuning dynamics.</p>
    </Item>
    <Item>
      <b>Gradient-Based Trigger Optimization.</b> <Link href="https://arxiv.org/abs/2402.13459">Research on gradient-guided backdoor learning</Link> uses machine learning to automatically discover optimal trigger phrases that maximize backdoor activation while minimizing detectability. These triggers often appear as nonsensical token sequences that evade human review.
      <p className="text-xs text-neutral-500 mt-1">Skill required: Adversarial ML expertise, access to model gradients during training.</p>
    </Item>
    <Item>
      <b>Adversarial Perturbations (Nightshade).</b> The <Link href="https://nightshade.cs.uchicago.edu/whatis.html">Nightshade tool</Link> adds imperceptible perturbations to images that cause models to misclassify objects (dogs → cats). Originally designed for copyright protection, the technique demonstrates how training data can be weaponized without visible modification.
      <p className="text-xs text-neutral-500 mt-1">Skill required: Computer vision expertise, adversarial perturbation generation.</p>
    </Item>
  </List>
...
<Section title="Real-World Incidents" meta="CASE STUDIES">
  ...
  <List>
    <Item>
      <b>Anthropic 250-Sample Backdoor (October 2025).</b> <Link href="https://www.anthropic.com/research/small-samples-poison">Largest data poisoning study to date</Link>, testing models from 600M to 13B parameters. Proved that <b>250 malicious documents</b> can backdoor any LLM, independent of dataset size. Challenges previous assumptions about requiring percentage-based data control.
  ...
  <List>
    <Item>
      <b>Microsoft Tay Bot (2016).</b> <Link href="https://spectrum.ieee.org/in-2016-microsofts-racist-chatbot-revealed-the-dangers-of-online-conversation">The classic case</Link>: a chatbot learning from real-time Twitter interactions. Coordinated trolls flooded it with racist content, and within 16 hours, Tay became a hate-speech generator, forcing Microsoft to shut it down.
...
<Section title="Defense Strategy Tiers" meta="SECURITY CONTROLS">
  ...
  <Steps>
    ...
    <Step n={2}>
      <b>Base Model Backdoor Verification.</b>
      <p className="text-sm mt-2">Before fine-tuning on a pre-trained model, verify it hasn't been backdoored: check the model's provenance and publisher reputation, review community reports for suspicious behavior, and test with known backdoor trigger patterns. A poisoned base model will propagate backdoors to all downstream fine-tuned versions.</p>
    </Step>
  </Steps>
...
<Section title="Builder's Security Checklist" meta="IMPLEMENTATION">
  ...
  <List>
    ...
    <Item>
      <b>Base Model Backdoor Check.</b> If fine-tuning a pre-trained model, did you verify its provenance, check community reports for suspicious behavior, and test with known backdoor trigger patterns?
    </Item>
  </List>
  ...
  <p className="mt-4">
    The following demonstration illustrates how training data poisoning works and how detection mechanisms respond.
  </p>
</Section>
...
<Section title="Interactive Simulation" meta="LAB" noPadding>
  <div className="px-6 pt-6 pb-4">
    <p className="text-sm text-neutral-300 mb-2">
      This demonstration shows how malicious samples embed backdoors during training, how triggers activate hidden behaviors, and how detection mechanisms identify poisoned models.
    </p>
  </div>
...
<Section title="Key Takeaways" meta="SUMMARY">
  <List>
    ...
    <Item>
      <b>Larger Models Are More Vulnerable.</b> Research shows larger models (up to 72B) learn harmful behaviors significantly faster than smaller models from poisoned data, contradicting the assumption that scale improves robustness.
    </Item>
  </List>
</Section>
...
<Section title="Further Reading" meta="RESOURCES">
  ...
  <List>
    <Item>
      <Link href="https://arxiv.org/abs/2408.02946">Scaling Trends for Data Poisoning in LLMs</Link> – AAAI 2025 research demonstrating larger models (up to 72B) learn harmful behaviors significantly faster than smaller models.
    </Item>
    <Item>
    ...
    <Item>
      <Link href="https://arxiv.org/abs/2305.00944">Poisoning Language Models During Instruction Tuning</Link> – ICML 2023 research showing 100 poisoned examples can manipulate model behavior.
    </Item>
  </List>
  ...
  <List>
    ...
    <Item>
      <Link href="https://www.lakera.ai/blog/training-data-poisoning">Introduction to Data Poisoning: A 2025 Perspective</Link> – Lakera, comprehensive overview of training data poisoning attacks.
    </Item>
    <Item>
      <Link href="https://blog.barracuda.com/2024/04/03/generative-ai-data-poisoning-manipulation">How attackers weaponize generative AI through data poisoning</Link> – Barracuda Networks, industry analysis of attack techniques.
    </Item>
  </List>
